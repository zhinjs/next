...
import { defaultOptions, supportedPluginExtensions } from "./constanse";
...
export function loadOptions(file: string): Partial<Options> {
  if (!fs.existsSync(file)) fs.writeFileSync(file, jsYaml.dump(defaultOptions));
  if (/\.ya?ml$/.test(file))
    return jsYaml.load(fs.readFileSync(file).toString()) as Partial<Options>;
